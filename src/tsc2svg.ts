//adapt from https://github.com/derhuerst/asciicast-to-svg
...
export async function tsc2svg(s: String, cols?: number | null, rows?: number | null, backgroundColor: number | null = 0) {
    ...
    for (let y = 0; y < terminal.rows; y++) {
        for (let x = 0; x < terminal.cols; x++) {
            ...
            const cell = terminal.buffer.normal.getLine(y)?.getCell(x)
    ...
    let vdom = h('svg', {
        ...
        viewBox: [-projectY(.25), -projectY(.25), svg_width, svg_height].join(' '),
        style: {
            backgroundColor: '#fff',
